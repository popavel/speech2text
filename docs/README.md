...
> the prose could be reviewed against its source side by side. The source files and `AGENTS.md`
> still carry the original long-form comments; they are trimmed to summaries and `Why:` pointers in
> the commits that follow. Until that lands, the [tripwire index](#tripwire-index) below is the
> checklist for the move, not a description of the tree.
...
One pointer per site. A declaration needing two anchors means the doc structure is wrong.
...
Every site that must keep an imperative warning in the code once the migration completes, the
prohibition it carries, and where its argument lives. This doubles as the acceptance criterion for
the move: nothing load-bearing has been lost if every row is present at its site and its anchor
holds the full reasoning.

| Site | Prohibition | Anchor |
| --- | --- | --- |
| `Updater.swift` — `@objc(updater:shouldPostpone…)` | Don't rename the Swift label or drop the pin | [distribution.md#relaunch-not-check](distribution.md#relaunch-not-check) |
...
| `Updater.swift` — `controller` | Don't delete the "unused" property | [distribution.md#the-seam](distribution.md#the-seam) |
| `Updater.swift` — auto-checks KVO | Don't add a second observer on `.standard` | [distribution.md#kvo-mirrors](distribution.md#kvo-mirrors) |
| `Updater.swift` — both KVO handlers | Never `MainActor.assumeIsolated` | [distribution.md#kvo-mirrors](distribution.md#kvo-mirrors) |
...
| `StallWatchdogTests.swift` — timing margins | Widen the budget, never delete the test | [testing.md#the-wall-clock-exception](testing.md#the-wall-clock-exception) |
...
| `Info.plist` — `SUFeedURL` | Permanent; changing it strands installed copies | [distribution.md#the-seam](distribution.md#the-seam) |
